...
function getFormData() {
    if (readStatus.checked) theStatus = true;
    if(nameField.value != '' && authorField.value != '' && numberPages.value != '') {
...
function addBookToLibrary(book) {
    ...
    buttonsDiv.innerHTML = `<div class="number${book.id}>
...
</div><div onclick="readFunction(this)" class="number${book.id}>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
<div onclick="removeFunction(this)" class="number${book.id}">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <title>Remove</title>
        <path fill="red" d="M20.41 19L22.54 21.12L21.12 22.54L19 20.41L16.88 22.54L15.47 21.12L17.59 19L15.47 16.88L16.88 15.47L19 17.59L21.12 15.47L22.54 16.88L20.41 19M19 13C20.09 13 21.12 13.3 22 13.81V6C22 4.89 21.11 4 20 4H4C2.9 4 2 4.89 2 6V18C2 19.11 2.9 20 4 20H13.09C13.04 19.67 13 19.34 13 19C13 15.69 15.69 13 19 13Z" />
    </svg>
</div>`
    ...
    /* numberId++ */
    if (theStatus) addGreen(div)
    
}

function addGreen(div) {
    div.classList.add('green')

}

function removeFunction(value) {
    let theId = value.className
    theId.slice(5)
    let p = parseInt(theId)
    myLibrary.splice(p, 1)
    value.parentNode.parentNode.remove()
}

function readFunction(value) {
    const containerDiv = value.parentNode.parentNode
    if(containerDiv.classList.contains('green')) {
            containerDiv.classList.remove('green')
            /* myLibrary[value.className.slice(-1)].read = false */
            console.log(value.className.slice(-1))
     } else {
            containerDiv.classList.add('green')
            /* myLibrary[value.className.slice(-1)].read = true */
            console.log(value.className.slice(-1))
     }
